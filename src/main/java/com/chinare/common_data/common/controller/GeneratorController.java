...
import com.chinare.common_data.common.service.GeneratorService;
import com.chinare.common_data.common.utils.GenUtils;
...
public class GeneratorController {
	...
	@RequestMapping("/code/{applicationName}/{tableName}")
	public void code(HttpServletRequest request, HttpServletResponse response,
			@PathVariable("applicationName") String applicationName, @PathVariable("tableName") String tableName)
			throws IOException {
		String[] tableNames = new String[] { tableName };
		byte[] data = generatorService.generatorCode(tableNames);
		response.reset();
		response.setHeader("Content-Disposition", "attachment; filename=\""+applicationName+"_"+tableName+".zip\"");
	...
	@RequestMapping("/batchCode/{applicationName}")
	public void batchCode(HttpServletRequest request, HttpServletResponse response,
			@PathVariable("applicationName") String applicationName, String tables) throws IOException {
		String[] tableNames = new String[] {};
		tableNames = JSON.parseArray(tables).toArray(tableNames);
		byte[] data = generatorService.generatorCode(tableNames);
		response.reset();
		response.setHeader("Content-Disposition", "attachment; filename=\""+applicationName+".zip\"");
		response.addHeader("Content-Length", "" + data.length);
		response.setContentType("application/octet-stream; charset=UTF-8");
